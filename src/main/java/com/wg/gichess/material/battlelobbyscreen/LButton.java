...
public class LButton extends Material {
    ...
    public LButton(int x, int y, BattleLobbyScreen screen) {
        super("textures/battlelobbyscreen/lbutton.png", x, y);
    ...
    public void render() {
        ...
        if(isInTransition){
            poseStack.pushPose();
            {
                poseStack.translate(-width*2 + transitionStartTime*width*4 ,0,1000);
                transition.render();

                transitionStartTime += delta/20;

                if(transitionStartTime >= 0.5 && !hasToggleMode){
                    screen.mode = screen.mode > 1 ? screen.mode - 1 : BattleLobbyScreen.maxMode;
