...
public class FundamentalsButton extends Movable {

    public int target;
    public int id=1;
    public FastScreen lastScreen;

    public FundamentalsButton( int x, int y,boolean isFirst,int target,FastScreen lastScreen) {
        super("textures/tutorialscreen/fundamentalsbutton.png", x, y,isFirst);
    ...
    public boolean click() {
        if (isMouseOn() && target != id){
            Minecraft.getInstance().setScreen(new FundamentalsScreen(false,id,lastScreen));
            return true;
        }
        return false;
    }
