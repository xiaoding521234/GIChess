...
public class TutorialButton extends Movable {
    public TutorialButton(int x, int y, boolean isFirst) {
        super("textures/battlescreen/tutorialbutton.png", x, y, isFirst);
        this.vY = 160;
    ...
    public boolean click() {
        if(isMouseOn()){
            Minecraft.getInstance().setScreen(new FundamentalsScreen(true,1, (FastScreen) Minecraft.getInstance().screen));
            return true;
        }
        return false;
    }
