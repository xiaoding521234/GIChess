...
import com.wg.gichess.screen.bagscrren.PropScreen;
import net.minecraft.client.Minecraft;

public class PropButton extends Movable {

    public int target;
    public int id=3;
    public FastScreen lastScreen;

    public PropButton( int x, int y,boolean isFirst,int target,FastScreen lastScreen) {
        super("textures/bagscreen/propbutton.png", x, y,isFirst);
    ...
    public boolean click() {
        if (isMouseOn() && target != id){
            Minecraft.getInstance().setScreen(new PropScreen(false,id,lastScreen));
            return true;
        }
        return false;
    }
